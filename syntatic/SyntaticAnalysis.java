...
public class SyntaticAnalysis {
    ...
    private void showError() {
        ...
        switch (current.type) {
            ...
            case UNEXPECTED_EOF:
                ...
                break;
            case END_OF_FILE:
                System.out.printf("Fim de arquivo\n");
                break;
            default:
    ...
    private AssertCommand procAssert() {// pelucivia
        ...
        eat(TokenType.CLOSE_PAR);
        eat(TokenType.SEMICOLON);
        AssertCommand assertcmd = new AssertCommand(line, expr, msg);
        return assertcmd;
    }
    ...
    private IfCommand procIf() {
        ...
        IfCommand ifcmd = new IfCommand(line, expr, thenCmd, elseCmd);
        return ifcmd;
    }
    ...
    private DoWhileCommand procDoWhile() {
        eat(TokenType.DO);
        int line = lex.getLine();
        Command body = procBody();
        eat(TokenType.WHILE);
        eat(TokenType.OPEN_PAR);
        Expr expr = procExpr();
        eat(TokenType.CLOSE_PAR);
        eat(TokenType.SEMICOLON);
        DoWhileCommand dwcmd = new DoWhileCommand(line, body, expr);
        return dwcmd;
    }
    ...
    // <term> ::= <prefix> { ( '*' | '/' | '%' ) <prefix> }
    private Expr procTerm() {// pelucivia
    ...
    // <list> ::= '[' [ <l-elem> { ',' <l-elem>} ] ']'
    private ListExpr procList() {// Prof revisou
        eat(TokenType.OPEN_BRA);
        int line = lex.getLine();
        ListExpr lexpr = new ListExpr(line);
        if (current.type == TokenType.NOT ||
                current.type == TokenType.SUB ||
                current.type == TokenType.INC ||
                current.type == TokenType.DEC ||
    ...
    private ListItem procLElem() {
        ...
        if (current.type == TokenType.NOT ||
                ...
                current.type == TokenType.OPEN_CUR) {
            li = procLSingle();
        } else if (current.type == TokenType.SPREAD) {
            li = procLSpread();
    ...
    // <l-single> ::= <expr>
    private SingleListItem procLSingle() {//pelucivia
        int line = lex.getLine();
        Expr expr = procExpr();
        SingleListItem slItem = new SingleListItem(line, expr);
        return slItem;
    }

    // <l-spread> ::= '...' <expr>
    private SpreadListItem procLSpread() {//pelucivia
        int line = lex.getLine();
        eat(TokenType.SPREAD);
        Expr expr = procExpr();
        SpreadListItem slItem = new SpreadListItem(line, expr);
        return slItem;
    }

    // <l-if> ::= if '(' <expr> ')' <l-elem> [ else <l-elem> ]
    private IfListItem procLIf() {// pelucivia
        int line = lex.getLine();
        eat(TokenType.IF);
        eat(TokenType.OPEN_PAR);
        Expr expr = procExpr();
        ...
        IfListItem ilitem = new IfListItem(line, expr, thenItem, elseItem);
        return ilitem;
    }

    // <l-for> ::= for '(' <name> in <expr> ')' <l-elem>
    private ForListItem procLFor() {// pelucivia
        int line = lex.getLine();
        eat(TokenType.FOR);
        eat(TokenType.OPEN_PAR);
        Variable var = procName();
        ...
        eat(TokenType.CLOSE_PAR);
        ListItem item = procLElem();
        ForListItem flItem = new ForListItem(line, var, expr, item);
        return flItem;
    }
